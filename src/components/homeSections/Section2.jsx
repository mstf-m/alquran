    ...
    <section className="h-screen md:pt-11 relative flex flex-col">
      <FramerTrigger position={200} style={'h-full'}>
        ...
        <div className="flex flex-col items-end bg-cream-gradient-mobile md:bg-cream-gradient container float-right md:rounded-s-3xl h-full md:h-fit">
          ...
          {/* need change */}
          <div className={`flex flex-col-reverse md:flex-row items-center md:items-start min-h-[80%] md:min-h-fit border-[10px] border-black/10 rounded-3xl bg-card-gradient bg-white self-center md:translate-y-8 mx-5 md:mx-0  ${isJustifyStart ? 'justify-start' : 'justify-center'}`}>
            {children}
          </div>
        </div>
      </FramerTrigger>
